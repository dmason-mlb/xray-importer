#!/usr/bin/env python3
"""
Resume labeling XRAY tests from where it left off
Skips already labeled tests and continues with the remaining ones
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('resume_labeling.log'),
...
class XrayTestLabelOrganizer:
    ...
    def get_imported_tests(self) -> List[Dict]:
        ...
        logger.info(f"Found {len(all_tests)} tests total")
        return all_tests
    ...
    def organize_tests(self, dry_run: bool = False):
        ...
        # Count already labeled tests
        for test in tests:
            existing_labels = test['fields'].get('labels', [])
            if self.has_organizational_labels(existing_labels):
                self.stats['already_labeled'] += 1
        
        logger.info(f"Already labeled: {self.stats['already_labeled']} tests")
        logger.info(f"Need to label: {self.stats['total'] - self.stats['already_labeled']} tests")
        ...
        for i, test in enumerate(tests):
            ...
            if (i + 1) % 50 == 0:
                ...
                remaining = (self.stats['total'] - self.stats['already_labeled'] - self.stats['success']) / rate if rate > 0 else 0
                logger.info(f"Progress: {i + 1}/{len(tests)} tests processed ({self.stats['success']} labeled, {rate:.1f} tests/sec, ~{remaining:.0f}s remaining)")
    ...
    def print_summary(self):
        ...
        logger.info(f"Already labeled: {self.stats['already_labeled']}")
...
def main():
    parser = argparse.ArgumentParser(description='Resume organizing XRAY tests using labels')
    ...
    logger.info("Starting XRAY test label organization (resume mode)")
