#!/usr/bin/env python3
"""
Working Phase 1 Assessment
Using the proven working query patterns from exploration
"""
...
def get_framed_tests_detailed(token):
    """Get detailed FRAMED tests using working query pattern"""
    ...
    # Use the working JQL format 
    jql = "project = FRAMED"
    
    print(f"📊 Fetching detailed tests with JQL: {jql}")
    ...
    print(f"✅ Retrieved {len(all_tests)} detailed tests")
    return all_tests

def get_framed_preconditions(token):
...
def complete_working_assessment():
    """Complete the Phase 1 assessment using working queries"""
    
    print("🔍 Starting Working Phase 1 Assessment")
    ...
    # Get all tests
    tests = get_framed_tests_detailed(token)
    
    # Get all preconditions  
    print("📋 Fetching all preconditions...")
    preconditions = get_framed_preconditions(token)
...
if __name__ == "__main__":
    try:
        analysis = complete_working_assessment()
